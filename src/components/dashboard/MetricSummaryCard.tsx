...
import ResponsivenessTrendModal from './ResponsivenessTrendModal';
...
const MetricSummaryCard: React.FC<MetricSummaryCardProps> = ({
  ...
  tooltip,
  trendData,
  responsivenessTrendData,
  description,
  subLabel,
  onCardClick,
}) => {
  const [isResponsivenessTrendOpen, setIsResponsivenessTrendOpen] = useState(false);

  const handleCardClick = () => {
    if (icon === 'responsiveness' && responsivenessTrendData) {
      setIsResponsivenessTrendOpen(true);
    } else if (onCardClick) {
      onCardClick();
    }
  };
    ...
    <>
      <div
        ...
        className={`bg-white rounded-lg shadow-sm border border-gray-200 p-5 h-full transition-all duration-300 ${
          (onCardClick || (icon === 'responsiveness' && responsivenessTrendData)) ? 'cursor-pointer transform transition-transform duration-300 hover:shadow-md hover:-translate-y-1' : ''
        }`}
        title={(trendData || (icon === 'responsiveness' && responsivenessTrendData)) ? "Click to view trend" : undefined}
      >
      ...
      {icon === 'responsiveness' && responsivenessTrendData && (
        <ResponsivenessTrendModal
          isOpen={isResponsivenessTrendOpen}
          onClose={() => setIsResponsivenessTrendOpen(false)}
