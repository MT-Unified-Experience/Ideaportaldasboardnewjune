    ...
    Papa.parse(csvData, {
      ...
      complete: (results) => {
        try {
          ...
          rows.forEach(row => {
            // Validate required fields
            if (!row.quarter || !row.year || !row.collaborative_ideas || !row.total_ideas) {
              return; // Skip invalid rows
            }

            const collaborativeIdeas = safeNumberConversion(row.collaborative_ideas);
            const totalIdeas = safeNumberConversion(row.total_ideas);
            const collaborationRate = row.collaboration_rate ? 
